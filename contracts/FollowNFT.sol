...
contract FollowNFT is AgoraNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    function initialize(uint256 profileId, address hub) external override {
    ...
    function name() public view override returns (string memory) {
        string memory handle = IAgoraHub(HUB).getHandle(_profileId);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IAgoraHub(HUB).getFollowNFTURI(_profileId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address followModule = IAgoraHub(HUB).getFollowModule(_profileId);

        _moveDelegate(fromDelegatee, toDelegatee, 1);

        super._beforeTokenTransfer(from, to, tokenId);
        IAgoraHub(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);
