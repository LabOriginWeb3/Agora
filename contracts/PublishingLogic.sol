...
library PublishingLogic {
    ...
    function createProfile(
        ...
        _profileById[profileId].gender = vars.gender;
        _profileById[profileId].education = vars.education;
        _profileById[profileId].company = vars.company;
        _profileById[profileId].bio = vars.bio;
        _profileById[profileId].email = vars.email;
        _profileById[profileId].twitter = vars.twitter;
        _profileById[profileId].discord = vars.discord;
        _profileById[profileId].interests = vars.interests;
        _profileById[profileId].link = vars.link;
        _profileById[profileId].location = vars.location;
        _profileById[profileId].agoraProfileId = vars.lensProfileId;
    ...
    function createPost(
        ...
        _pubByIdByProfile[vars.profileId][pubId].contentType = vars.contentType;

        _pubByIdByProfile[vars.profileId][pubId].advertisement = vars
            .advertisement;
        _pubByIdByProfile[vars.profileId][pubId].adDeadline = vars.adDeadline;
        if (
            vars.contentType == DataTypes.ContentType.Question ||
            vars.contentType == DataTypes.ContentType.BetQuestion
        ) {
            _profileById[vars.profileId].questions++;
        }

        if (
            vars.contentType == DataTypes.ContentType.BetQuestion &&
            !Strings.equal(vars.options, "")
        ) {
            _pubByIdByProfile[vars.profileId][pubId].options = vars.options;
        }
    ...
    function createComment(
        ...
        _pubByIdByProfile[vars.profileId][pubId].pubIdPointed = vars
            .pubIdPointed;
        _pubByIdByProfile[vars.profileId][pubId].contentType = vars.contentType;
        if (vars.contentType == DataTypes.ContentType.BetAnswer) {
            _profileById[vars.profileId].bet++;
            _profileById[vars.profileId].answers++;
        }
        // _profileById[vars.profileId].interactedContents++;
        // Collect Module Initialization
        bytes memory collectModuleReturnData = _initPubCollectModule(
            vars.profileId,
            pubId,
